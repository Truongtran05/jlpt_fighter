...
export default function AccountPage() {
  ...
  const formatDate = (value) => value
    ? new Intl.DateTimeFormat(language === "vi" ? "vi-VN" : "en-US", { dateStyle: "medium" }).format(new Date(value))
    : t("No data")
    ...
    return (
      <FullScreenVSection justify="center">
        <Box bg="white" borderWidth="1px" borderColor="bushido.error" borderRadius="4px" p={6}>
          <Heading size="md">{t("Unable to load account")}</Heading>
          <Text color="bushido.muted" mt={2}>{t(error)}</Text>
        </Box>
      </FullScreenVSection>
    )
  }
  ...
  if (!accountData) {
    return <FullScreenVSection justify="center"><Text color="bushido.muted">{t("Loading profile...")}</Text></FullScreenVSection>
  }
  ...
  const stats = [
    { label: t("Flashcard sets"), value: accountData.total_flashcard_sets, note: t("Active sets"), icon: LuLayers },
    { label: t("Total cards"), value: totalCards, note: t("In your library"), icon: LuBookOpen },
    { label: t("Remembered"), value: completedCards, note: `${mastery}% ${t("of all cards")}`, icon: LuCheck },
    { label: t("Review due"), value: accountData.total_incomplete_flashcards, note: t("Cards not remembered"), icon: LuClock },
  ]
  const categories = [
    { label: "Kanji", value: accountData.total_kanji_flashcards, icon: LuBrain },
    { label: t("Vocabulary"), value: accountData.total_vocabulary_flashcards, icon: LuLanguages },
    { label: t("Grammar"), value: accountData.total_grammar_flashcards, icon: LuBookOpen },
  ]
    ...
    <FullScreenVSection gap={6}>
      <Box bg="bushido.surfaceContainer" borderRadius="4px" p={{ base: 5, md: 8 }}>
        <HStack gap={{ base: 4, md: 6 }} align="center">
          ...
          <VStack align="flex-start" gap={1} minW={0}>
            <Text fontFamily="mono" fontSize="12px" fontWeight="600" letterSpacing="0.05em" color="bushido.primary">{t("FIGHTER PROFILE")}</Text>
            ...
            <HStack color="bushido.muted" fontSize="13px" gap={2}><LuCalendarDays /><Text>{t("Member since")} {formatDate(accountData.account_created_at)}</Text></HStack>
          </VStack>
        </HStack>
      </Box>
      ...
      <Box display="grid" gridTemplateColumns={{ base: "1fr", lg: "minmax(0, 2fr) minmax(280px, 1fr)" }} gap={6} alignItems="start">
        <Box bg="white" borderWidth="1px" borderRadius="4px" p={{ base: 5, md: 6 }} _hover={{"transform": "translateY(-1px)", boxShadow: "0 4px 6px rgba(0, 0, 0, 0.1)"}} transition="all .2s">
          <Heading as="h2" fontSize="20px" lineHeight="28px">{t("Library progress")}</Heading>
          <Text color="bushido.muted" fontSize="14px" mt={1}>{t("Breakdown of your")} {totalCards} {t("flashcards")}</Text>
              ...
              return (
                <Box key={label} >
                  <HStack justify="space-between" mb={2}>
                    ...
                    <Text fontFamily="mono" fontSize="12px" color="bushido.muted">{value} {t("CARDS")} · {percentage}%</Text>
                  </HStack>
        ...
        <VStack align="stretch" gap={4}>
          <Box bg="white" borderWidth="1px" borderRadius="4px" p={5} _hover={{"transform": "translateY(-1px)", boxShadow: "0 4px 6px rgba(0, 0, 0, 0.1)"}} transition="all .2s">
            <Heading as="h2" fontSize="20px" lineHeight="28px">{t("Account")}</Heading>
            <VStack align="stretch" gap={4} mt={5}>
              ...
              <HStack align="flex-start"><Box color="bushido.primary" pt={1}><LuClock /></Box><Box><Text fontSize="12px" fontFamily="mono" color="bushido.muted">{t("LAST LOGIN")}</Text><Text>{formatDate(accountData.last_login)}</Text></Box></HStack>
            </VStack>
          </Box>
          <Button variant="outline" borderColor="bushido.error" color="bushido.error" borderRadius="8px" onClick={handleLogout} loading={isLoggingOut} loadingText={t("Logging out")}><LuLogOut />{t("Log out")}</Button>
        </VStack>
      </Box>
    </FullScreenVSection>
  )
